...
Decentralized identifiers for AI agents — timestamped proof of when an agent was created, its capabilities, its provenance. Mint/verify/revoke on testnet. **Customer questions:** Who buys agent identity — the platform, the enterprise deploying agents, or the regulator-facing buyer? Who feels the pain of NOT having it? Competitors: **ERC-8004 (Ethereum's Identity / Reputation / Validation registries — now the standard to assume wins for registration & certification)**, Cronos (Proof of Identity), Story Protocol (ATCP/IP), W3C DID. **The threat is now concrete:** ERC-8004 + x402 are commoditizing static agent identity and payment. **Our response is compose-not-compete** — be ERC-8004-compatible and differentiate on what they lack: *as-of-T authority* (was the agent allowed to act at time T), neutral cross-party ordering, and the receipt of the act. Probe whether the buyer's pain is *identity* (covered) or *independent time / ordering / a provable receipt* (still empty).

### 3. Smart Contracts & Receipts
Verifiable on-chain receipts for agent actions — proof an agent did X at time T with inputs/outputs, in a neutral order a counterparty will accept. **Customer questions:** Who pays for a receipt specifically (not identity, not timestamps)? Compliance, audit, inter-agent trust, **insurance claims**? Competitors: EAS, Verax, **ERC-8004 Validation/Reputation** (general attestations). Threat: attestation commoditizes — so the edge is **independent time + neutral cross-party ordering + a claims-grade record of the act**, not the attestation itself.
...
| **ERC-8004 (Ethereum)** | Agent Identity / Reputation / Validation registries — the emerging standard for registration & certification | Covers *who / whether-trusted*, not *when / in-what-order / authorized-at-T*. **We compose with it (ERC-8004-compatible), not compete.** Our layer is independent time + the receipt of the act. |
| **x402** | Agent-to-agent stablecoin payment over HTTP | Moves the money; produces no neutral, independently-timed receipt of the payment context. An x402 call is a receipt-worthy event for us. |
...
**Claimed wedge:** with ERC-8004 + x402 commoditizing identity and payment, the differentiator is **not** identity issuance — it is **independent time, neutral cross-party ordering, as-of-T authority, and a claims-grade receipt of the act**, anchored on a patented time oracle and a validator network no single party controls. Treat it as a hypothesis to validate against a named buyer's pain — current discovery read: **no demand evidence yet** — not a given.
